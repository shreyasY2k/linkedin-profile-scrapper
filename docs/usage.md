...
Everything the README's [Setup](../README.md#setup) section points at: minting a
token, storing a session, fetching a profile, what happens when LinkedIn will
not answer, proving that two callers are isolated, the opt-in live check,
resetting the stack, running the tests, scanning for secrets, and the full
configuration reference.

Every command below assumes a **local** stack that is already up
(`docker compose up -d --wait` — see
[Run it locally](../README.md#run-it-locally)). The deployed equivalents, which
depend on no local file at all, are the two graded commands in the
...
no local file at all, are in the README's
[Quick start](../README.md#quick-start). The first line loads
...
scored on; these are their local twins.
...
| The evaluator client secret | A **real** credential, published on purpose — see [About that client secret](../README.md#about-that-client-secret) |
