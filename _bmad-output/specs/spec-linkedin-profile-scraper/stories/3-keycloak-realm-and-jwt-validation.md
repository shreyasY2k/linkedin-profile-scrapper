...
20. `app/main.py`, `app/auth.py` — two comments asserted couplings that do not exist: that handler registration must precede router inclusion (Starlette resolves the handler table per request), and that `TOKEN_URL` is quoted verbatim in the README and read by tests (nothing referenced it). Both corrected to what is actually true — `TOKEN_URL` feeds the OpenAPI description of the security scheme (story 5 also made it that scheme's `tokenUrl`, so it is now load-bearing rather than descriptive).
...
**OpenAPI**, on the same probe: `components.securitySchemes.KeycloakClientCredentials` is `{type: oauth2}` with a single `clientCredentials` flow whose `tokenUrl` is the **external** issuer (`${KEYCLOAK_ISSUER_URL}/realms/${KEYCLOAK_REALM}/protocol/openid-connect/token`), the `/api/v1` route carries `security: [{"KeycloakClientCredentials": []}]` and a 401 response referencing `#/components/schemas/ErrorEnvelope`, and `/health` carries no `security` key at all.

> **Superseded by story 5 (2026-08-27).** This story shipped an `http`/`bearer` scheme named `KeycloakBearer`. Story 5 replaced it with the OAuth2 client-credentials scheme described above, deliberately and with this story's tests updated rather than weakened: a bare bearer scheme renders in `/docs` as a box to paste an already-minted token into, which is useless to an evaluator who has not minted one yet. Nothing about *validation* changed — only the document's description of where a token comes from. The half of this assertion that did NOT change, and must not, is the last clause: `/health` carries no `security` key.
