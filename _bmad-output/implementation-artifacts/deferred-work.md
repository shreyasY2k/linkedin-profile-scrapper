...
- source_spec: `_bmad-output/specs/spec-linkedin-profile-scraper/stories/3-keycloak-realm-and-jwt-validation.md`
  ...
  status: NOT RESOLVED by story 8, deliberately, and the attempt is instructive. The 400/BAD_REQUEST row was dropped on the reasoning that INVALID_URL now owns every 400 - true of every 400 this codebase RAISES and false of the ones it does not: FastAPI's own body-read guard raises HTTPException(400) before any of our code runs, so `PUT /api/v1/session` with an unparseable body rendered code INTERNAL_ERROR at a 400. Reproduced in review and reverted; a test now drives an unparseable body through a real route. The set stands at 400/404/405/422/503 and the disjointness test is untouched. The standing guidance is narrower than this entry originally implied: a fallback row is superseded only when the status is unreachable, which is a proof about a framework's internals rather than about this repository.
...
- source_spec: `_bmad-output/specs/spec-linkedin-profile-scraper/stories/2-walking-skeleton-deploy.md`
  ...
  status: PARTIALLY RESOLVED by story 8, and the claim is now stated honestly. app/api/v1/install_unmatched_path_guard registers catch-all routes at /api/v1 and /api/v1/{unmatched_path:path} carrying the same require_claims dependency, mounted last in create_app; they match on path alone (AnyMethodRoute) so no verb - including invented ones - short-circuits past the boundary into Starlette's own 405, whose Allow header used to name exactly which methods each real route answered. An unauthenticated request under the prefix is now 401 whether or not the path is real, an authenticated caller still gets a real 404, a wrong method still gets 405 with Allow, and a trailing slash on a real route still gets its 307. What this is NOT is enumeration resistance: /openapi.json is unauthenticated and publishes every route (story 9 ships it as the API documentation), and the body-parsing leak below remains. The value is one error envelope and one status for every unauthenticated request under the prefix, which is what the README now claims and nothing more.
...
- source_spec: `_bmad-output/specs/spec-linkedin-profile-scraper/stories/8-error-taxonomy-and-handlers.md`
  summary: The `me`-wall reclassification is evidenced in one direction only, so a healthy cookie walled on `me` would be reported as expired.
  evidence: Story 4 verified live that a DEAD li_at draws a 200 authwall from `me`, which is what the new SESSION_EXPIRED classification rests on. Nothing establishes the converse - that LinkedIn never walls `me` for a HEALTHY session from a datacenter IP - and if it does, `PUT /api/v1/session` answers `last_use_ok: false` and sends the caller to replace a credential that works. The blast radius is bounded: `me` is reached only by the PUT verification and the opt-in live check, never by the profile route, so a wrong verdict costs a misleading bookkeeping field rather than a failed fetch. Revisit if a healthy session is ever observed drawing a wall on `me`; the fix would be to require a second signal before reporting expiry.

- source_spec: `_bmad-output/specs/spec-linkedin-profile-scraper/stories/8-error-taxonomy-and-handlers.md`
  summary: HEAD and OPTIONS under /api/v1 are now behind authentication, so a future browser preflight gets 401 rather than 405.
  evidence: The unmatched-path guard matches every method, which is what closes the verb-based enumeration leak - but it also means an OPTIONS preflight to any /api/v1 path is answered 401 by `require_claims` before anything CORS-aware runs. There is no CORS middleware today and no browser client, so nothing is broken now. Whoever adds a browser lane must install CORSMiddleware ahead of the router (it short-circuits OPTIONS itself) rather than carving an exception into the guard.

- source_spec: `_bmad-output/specs/spec-linkedin-profile-scraper/stories/8-error-taxonomy-and-handlers.md`
  summary: An unparseable request body is answered 400/422 while an absent path is answered 401, so route existence is still partially observable without a token.
  evidence: FastAPI reads and parses the body before route dependencies run (`fastapi/routing.py`), so `PUT /api/v1/session` with malformed bytes returns 400 BAD_REQUEST while the same request to a path with no route returns 401. Accepted by the author rather than fixed: closing it means moving authentication into middleware ahead of body parsing, which is a larger change than the leak justifies, and `/openapi.json` publishes the full route list unauthenticated anyway - enumeration resistance is not a property this service claims. The README says so plainly rather than implying otherwise.

- source_spec: `_bmad-output/specs/spec-linkedin-profile-scraper/stories/9-readme-secret-audit-final-deploy.md`
  summary: Automated re-login to mint a fresh `li_at` when one expires was asked for and declined; a service-level fallback session (`SERVICE_LI_AT`) is the shape that should be built instead.
  evidence: Re-login addresses cookie expiry, but the dominant failure here is LinkedIn refusing a datacenter IP - a freshly minted cookie meets the same authwall on the next fetch, so the machinery would be built and stale-serve would still be carrying the load. Building it needs the demoted headless browser (400-700 MB against a footprint already near the instance ceiling), and it means storing a username and password reversibly encrypted, since they must be replayable: a leaked cookie is one session, a leaked password is the account. It also escalates the terms-of-service position from automated collection to automated login, which LinkedIn restricts far faster. `SPEC.md` already lists both "automated session refresh or programmatic re-login" and "defeating challenge pages, CAPTCHAs, or IP-reputation blocks" as non-goals. The achievable half of the request is a `SERVICE_LI_AT` env var used only when a caller has stored no session of their own, making renewal an env edit rather than a remembered `curl`. Declined for this submission because it rewords CAP-4 and makes `NO_SESSION` unreachable on that path - a contract change two days from a deadline where contract fidelity is itself graded. Recorded in the README's Known limitations as the first thing this service should grow.
